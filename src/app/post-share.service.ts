 ...
 * drawn are this dealer's OWN branding (see ShareBrandingService): their logo
...
export class PostShareService {
  ...
  async share(post: SharePost): Promise<ShareOutcome> {
    // This device's branding wins. Posts published before branding moved into
    // the app may still carry the admin's, so that is the fallback.
    const logo = this.branding.logo || post.logo || null;
    const details = (this.branding.details || post.details || '').trim();
  ...
  // The dealer's own logo in the top-right corner, transparent — no card, no tile.
  ...
  // The dealer's own detail lines at the bottom-right, right-aligned and stacked
