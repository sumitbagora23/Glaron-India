...
export class ProductService {
  ...
  addProduct(productData: Omit<Product, 'status'>) {
    ...
    if (this.firestore) {
      setDoc(doc(this.firestore, 'products', newProduct.id), newProduct)
        .catch(err => console.warn('Firestore add notice:', err?.message || err));
    }
  }
  ...
  updateProduct(updatedProduct: Product) {
    ...
    if (this.firestore) {
      setDoc(doc(this.firestore, 'products', product.id), product)
        .catch(err => console.warn('Firestore update notice:', err?.message || err));
    }
  }
